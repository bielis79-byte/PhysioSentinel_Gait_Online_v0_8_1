...
def _support_mask_2d(seg, side, fps, expected_stride_s=np.nan):
    """
    v0.8.1: estimación cinemática 2D de apoyo con continuidad temporal.
...
def _automatic_straight_walking_mask(seg, fps):
    """
    v0.8.1: detecta y excluye automáticamente transiciones/giro de 180° en
...
def _cycle_timing_metrics(L, R, fps):
    """
    v0.8.1 · Estimador temporal coherente con apoyo/doble apoyo.

    Cadencia y CV se derivan de las duraciones IC→IC de los mismos ciclos
    rectilíneos validados utilizados para stance/swing.
    """
    l_cycles = np.asarray(L.get("cycle", []), dtype=float)
    r_cycles = np.asarray(R.get("cycle", []), dtype=float)
    ...
    def _clean(x):
        ...
        lo = max(0.55, 0.65 * med)
        hi = min(2.50, 1.45 * med)
        return x[(x >= lo) & (x <= hi)]

    l = _clean(l_cycles)
    r = _clean(r_cycles)
    pooled = np.concatenate([l, r]) if len(l) + len(r) else np.asarray([], dtype=float)

    if len(pooled) < 3 or len(l) < 1 or len(r) < 1:
        return {
            "cadence": np.nan, "mean_stride": np.nan, "cv": np.nan,
            "l_mean_stride": np.nan, "r_mean_stride": np.nan,
            "n_cycles": int(len(pooled)), "quality": "No fiable",
            "reason": "menos de 3 ciclos IC→IC válidos con representación bilateral"
        }

    mean_stride = float(np.nanmean(pooled))
    cadence = float(120.0 / mean_stride) if mean_stride > 0 else np.nan
    cv = float(np.nanstd(pooled, ddof=1) / mean_stride * 100.0) if len(pooled) >= 3 else np.nan
    l_mean = float(np.nanmean(l)) if len(l) else np.nan
    r_mean = float(np.nanmean(r)) if len(r) else np.nan

    quality = "Alta" if len(pooled) >= 6 and len(l) >= 2 and len(r) >= 2 else "Moderada"
    return {
        "cadence": cadence,
        "mean_stride": mean_stride,
        "cv": cv,
        "l_mean_stride": l_mean,
        "r_mean_stride": r_mean,
        "n_cycles": int(len(pooled)),
        "quality": quality,
        "reason": f"{len(pooled)} ciclos IC→IC rectilíneos validados"
    }
...
def compute_gait_phase_metrics(seg, fps, is_frontal, expected_stride_s=np.nan,
    ...
    """
    v0.8.1: fases del ciclo a partir de estados de apoyo estabilizados.
    ...
    if reliable:
        # v0.8.1: el doble apoyo se calcula SOLO en el dominio cubierto
        ...
        ds_ok = (
            ...
            and flight_pct <= 5.0
            and (not np.isfinite(ds_discrepancy) or ds_discrepancy <= 10.0)
        )
        if ds_ok:
            ...
            phase_quality = "Parcialmente fiable · doble apoyo suprimido por control interno"
    else:
    ...
    out = [
        ...
        {"key":"double_support_pct_2d","label":"Doble apoyo estimado","value":double_pct,"unit":"% dominio válido","quality":phase_quality,"notes":"v0.8.1: intersección de apoyos dentro de los mismos ciclos rectilíneos que generan cadencia/CV; se suprime si aparece fase aérea o si no concuerda con la ocupación de apoyo."},
...
def compute_metrics(df, fps, start_frame, end_frame, view, assistive_device="Sin ayuda", scale_cm_per_px=0.0):
    ...
    # v0.8.1: cadencia y CV salen de LOS MISMOS ciclos IC→IC que generan
    # stance/swing. Los contactos bilaterales quedan como verificación secundaria.
    cycle_timing = _cycle_timing_metrics(l_cycle, r_cycle, fps) if support_reliable else {
        "cadence": np.nan, "mean_stride": np.nan, "cv": np.nan,
        "l_mean_stride": np.nan, "r_mean_stride": np.nan,
        "n_cycles": 0, "quality": "No fiable",
        "reason": "segmentación apoyo/oscilación insuficiente"
    }
    contact_metrics = _contact_step_metrics(l_cycle, r_cycle, fps) if support_reliable else {
        "cadence": np.nan, "mean_step": np.nan, "cv": np.nan, "asym": np.nan,
        "events": [], "n_intervals": 0, "quality": "No fiable",
        "reason": "segmentación apoyo/oscilación insuficiente"
    }

    cadence = cycle_timing["cadence"]
    mean_stride_cycle = cycle_timing["mean_stride"]
    cv_alt = cycle_timing["cv"]
    mean_alt = (mean_stride_cycle / 2.0) if np.isfinite(mean_stride_cycle) else np.nan

    contact_cadence = contact_metrics.get("cadence", np.nan)
    cadence_crosscheck_error = (
        abs(contact_cadence - cadence) / cadence * 100.0
        if np.isfinite(contact_cadence) and np.isfinite(cadence) and cadence > 0
        else np.nan
    )
    contact_agrees = np.isfinite(cadence_crosscheck_error) and cadence_crosscheck_error <= 10.0
    asym = contact_metrics["asym"] if contact_agrees else np.nan

    # Fallback solo para cadencia; CV y asimetría quedan anulados.
    if not np.isfinite(cadence) and len(intervals0) >= 3:
        mean_alt = float(np.nanmedian(intervals0))
        cadence = float(60.0 / mean_alt) if mean_alt > 0 else np.nan
        mean_stride_cycle = 2.0 * mean_alt if np.isfinite(mean_alt) else np.nan
        cv_alt = np.nan
        asym = np.nan
        cadence_crosscheck_error = np.nan
    ...
    # Consistencia temporal v0.8.1: usar el tiempo rectilíneo utilizable.
    segment_duration_s = float((seg.frame.max() - seg.frame.min() + 1) / fps)
    valid_duration_s = float(np.sum(straight_mask) / fps)
    detected_steps = int(len(contact_metrics.get("events", []))) if contact_metrics.get("events") else int(len(crossings))
    ...
    consistency_error = (abs(detected_steps - expected_steps) / max(expected_steps, 1.0) * 100.0) if np.isfinite(expected_steps) else np.nan
    if detected_steps < 4 or not np.isfinite(consistency_error):
    ...
    metrics = [
        ...
        {"key":"straight_walking_usable_pct","label":"Tramo rectilíneo utilizable","value":float(straight_info["usable_pct"]),"unit":"% segmento","quality":"Control interno","notes":straight_info["reason"]+"; v0.8.1 excluye giro/transiciones de las métricas temporales sensibles."},
        ...
        {"key":"cadence_exp","label":"Cadencia estimada","value":cadence,"unit":"pasos/min","quality":cycle_timing.get("quality","Experimental") if np.isfinite(cadence) else "No calculable","notes":"v0.8.1: 120 / duración media IC→IC de los mismos ciclos rectilíneos usados para apoyo/oscilación. El detector bilateral queda como control cruzado."},
        {"key":"step_events_detected","label":"Eventos de paso/alternancia detectados","value":float(detected_steps),"unit":"eventos","quality":"Experimental","notes":"Número de cruces distales D/I aceptados en el segmento; no son heel-strikes validados."},
        ...
        {"key":"stride_cycle_duration_s","label":"Duración media del ciclo IC→IC","value":mean_stride_cycle,"unit":"s","quality":cycle_timing.get("quality","No fiable"),"notes":"Base temporal primaria de cadencia, CV, apoyo y control de doble apoyo en v0.8.1."},
        ...
        {"key":"cadence_contact_crosscheck_error_pct","label":"Discrepancia cadencia ciclos vs contactos","value":cadence_crosscheck_error,"unit":"%","quality":"Alta" if np.isfinite(cadence_crosscheck_error) and cadence_crosscheck_error<=10 else ("Revisar" if np.isfinite(cadence_crosscheck_error) else "No calculable"),"notes":"Si supera 10%, la asimetría L→R/R→L se suprime y prevalece la cadencia de ciclos IC→IC."},
        ...
        {"key":"regularity_cv","label":"Variabilidad temporal del ciclo (CV)","value":cv_alt,"unit":"%","quality":cycle_timing.get("quality","Experimental") if np.isfinite(cv_alt) else "No calculable","notes":"v0.8.1: CV de las duraciones IC→IC de los mismos ciclos rectilíneos validados usados para apoyo/oscilación; no es CV de tiempo de paso de laboratorio."},
        {"key":"temporal_asymmetry_exp","label":"Asimetría temporal experimental","value":asym,"unit":"%","quality":"Experimental" if np.isfinite(asym) else "No calculable","notes":"v0.8.1: L→R frente a R→L solo se publica cuando la cadencia bilateral concuerda ≤10% con la cadencia de ciclos IC→IC."},
    ...
    if not is_frontal:
        ...
        if len(a)>20 and np.isfinite(mean_alt) and mean_alt>0:
            ...
            # v0.8.1: fase circular canónica en [-180, +180).
        ...
        metrics += [
            ...
            {"key":"trunk_pelvis_phase_deg","label":"Desfase tronco-pelvis estimado","value":phase_deg,"unit":"° ciclo","quality":"Experimental" if np.isfinite(phase_deg) else "No calculable","notes":"v0.8.1: desfase de correlación cruzada normalizado al ciclo IC→IC y expresado entre −180° y +180°; útil para seguimiento, no diagnóstico aislado."},
        ]
...
def generate_reports(metrics, view, patient_code, record_name, assistive_device):
    """Informe clínico estructurado v0.7.9 con control de consistencia temporal antes de informar stance/swing."""
    ...
    def ref_for(key):
        ...
        if bare == "regularity_cv":
            return "Sin umbral diagnóstico directamente transferible: en v0.8.1 este CV corresponde a duración de ciclo/zancada IC→IC en ciclos rectilíneos validados; no debe compararse directamente con umbrales publicados para CV de tiempo de paso."
        ...
        if bare == "trunk_pelvis_phase_deg":
            return "Sin banda normativa universal. Desfase expresado en grados de ciclo (360° = 1 ciclo completo); interpretar longitudinalmente y junto con la calidad del tracking."
    ...
    if np.isfinite(ds):
        lines.append(f"• Apoyo bipodal estimado: {ds:.1f}% del segmento/ciclo analizado (Ref. contextual: en marcha adulta habitual el doble apoyo total suele ocupar aproximadamente 20–24% del ciclo; la estimación v0.8.1 usa el mismo dominio de ciclos rectilíneos que cadencia/apoyo y se anula si falla el control físico interno).")
...
# ------------------------- UI -------------------------
st.title("PhysioSentinel Gait")
st.caption(f"Versión {APP_VERSION} · 2 cámaras · sincronización experimental · preparación 3D · Supabase tolerante a fallos")
        ...
        if st.button("▶ Analizar marcha", type="primary", use_container_width=True):
            try:
                ...
                if current_two:
                    ...
                    meta1 = st.session_state.meta1
                    meta2 = st.session_state.meta2
                    ...
                    ready, reasons = readiness_3d(current_two and st.session_state.mode or "", q1, q2, sq, st.session_state.get("selected_calibration_name"))
...
st.divider()
st.caption("PhysioSentinel Gait v0.8.1 · coherencia temporal ciclo-cadencia-apoyo · segmentación temporal v3 · stance por ciclos continuos · control de consistencia · informe técnico/paciente · 2 cámaras · preparación 3D")
